    ...
    const result = await store.downloadChatTemplate(
      ...
      (progress, downloaded, filename, alreadyExists) => {
        ...
        if (filename) {
          ...
          store.fetchChatTemplates()
        }
      }
    )

    if (!result && store.chatTemplateDownloadError) {
      downloadError.value = store.chatTemplateDownloadError
      showToast(store.chatTemplateDownloadError, 'error')
    }
  } catch (e) {
...
async function handleDelete(filename) {
  ...
  try {
    const result = await store.deleteChatTemplate(filename)
    if (result) {
      showToast(`Deleted ${filename}`, 'success')
    } else {
      showToast(store.chatTemplateDownloadError || 'Delete failed', 'error')
    }
  } catch (e) {
  ...
  <div class="m-2 space-y-6">
    ...
    <div class="card">
      ...
      <div class="flex items-center gap-3">
        <div class="relative flex-1">
          ...
          <input
            v-model="templateUrl"
            placeholder="Template URL (e.g., https://example.com/template.json)..."
      ...
      <div v-if="downloading" class="mt-3 space-y-1">
        <div class="flex items-center justify-between text-xs">
          <span class="text-text-muted">Downloading template...</span>
    ...
    <div class="space-y-3">
      <h3 class="text-xs font-semibold text-text-muted uppercase tracking-wider flex items-center gap-2">
        ...
        Saved Templates
        <span v-if="store.chatTemplates.length" class="badge bg-bg-tertiary text-text-muted text-xs">{{ store.chatTemplates.length }}</span>
      </h3>

      <!-- Empty State -->
      <div v-if="store.chatTemplates.length === 0" class="card flex flex-col items-center justify-center py-16 text-text-muted">
        <svg class="w-12 h-12 mb-4 text-text-muted/30" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        <p class="text-sm">No chat templates saved</p>
        <p class="text-xs mt-1">Download templates using the form above</p>
      </div>

      <!-- Template Cards -->
      <div
        v-for="template in store.chatTemplates"
        :key="template.filename"
        class="card"
      >
        <div class="flex items-center justify-between">
          <div class="min-w-0 flex-1">
            <h4 class="text-sm font-semibold text-text-primary truncate">{{ template.filename }}</h4>
          ...
          <button
            @click="handleDelete(template.filename)"
