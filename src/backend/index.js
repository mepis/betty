...
function getExports() {
  ...
  const exports = [
    ...
    `export LLAMA_ARG_FIT=${env.LLAMA_ARG_FIT ? "on" : "off"}`,
    ...(env.LLAMA_ARG_FIT && env.LLAMA_ARG_FIT_TARGET !== undefined && env.LLAMA_ARG_FIT_TARGET !== null
      ? [`export LLAMA_ARG_FIT_TARGET=${env.LLAMA_ARG_FIT_TARGET}`]
      : []),
    ...(env.LLAMA_ARG_FIT && env.LLAMA_ARG_FIT_CTX !== undefined && env.LLAMA_ARG_FIT_CTX !== null
...
function writeResultsToMarkdown() {
  ...
  md +=
    "| Test Run | GGML_CUDA_ENABLE_UNIFIED_MEMORY | CUDA_SCALE_LAUNCH_QUEUES | LLAMA_CACHE | GGML_CUDA_P2P | LLAMA_ARG_FIT |\n";
  md +=
    "|----------|---------------------------------|--------------------------|-------------|---------------|---------------|\n";

  for (const run of results) {
    const e = run.env;
    const fitVal = e.LLAMA_ARG_FIT === true ? "on" : e.LLAMA_ARG_FIT === false ? "off" : e.LLAMA_ARG_FIT;
    md += `| ${run.testRunId} | ${e.GGML_CUDA_ENABLE_UNIFIED_MEMORY} | ${e.CUDA_SCALE_LAUNCH_QUEUES} | ${e.LLAMA_CACHE} | ${e.GGML_CUDA_P2P} | ${fitVal} |\n`;
  }
