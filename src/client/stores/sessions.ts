...
export const useSessionStore = defineStore('sessions', () => {
  ...
  // Actions
  async function fetchSessions(page = 1, pageSize = 25): Promise<void> {
    loading.value = true;
    error.value = null;
    try {
  ...
  async function createSession(title?: string, branchFrom?: string): Promise<string> {
  ...
  function setCurrentSession(session: SessionDetail | null): void {
    currentSession.value = session;
  }

  async function sendMessage(content: string, sessionId?: string): Promise<Message> {
    if (!sessionId && !currentSession.value) throw new Error('No active session');
  ...
  function addAssistantMessage(sessionId: string, content: string): void {
    if (!currentSession.value || currentSession.value.session.id !== sessionId) return;
