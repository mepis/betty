...
# model=Qwen3.6-35B-A3B-UD-Q6_K_XL.gguf # mtp model
...
./llama-server -m $MODEL_DIR/$model --mmproj $MODEL_DIR/$mmproj --port $port --host $host -c $context -ngl 999 --cont-batching --temp $temp --top-p $topP --min-p $minP --top-k $topK --batch-size 1152 --ubatch-size 704 --flash-attn 1 --reasoning 1 --split-mode $splitMode --tensor-split $tensorSplit -e --presence-penalty 0.0 --reasoning-budget 1024 --reasoning-budget-message "Proceed to final answer." --cache-ram 0 --rope-scaling yarn --rope-scale 2.0 --parallel 2
